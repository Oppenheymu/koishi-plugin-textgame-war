...
import { 更新地区资料, 地区驻扎权限设置检查 } from "@/utils";
...
async function 执行驻扎权限设置(
	...
	const { username, 地区编号, 展示地区名称, 地区资料 } =
		await 地区驻扎权限设置检查(ctx, session, 地区编号参数);
...
export function 设置驻扎权限(ctx: Context) {
	ctx
		.command("设置驻扎权限 <权限模式:string> [地区编号:string]")
		.alias("地区驻扎权限")
		.alias("设置地区驻扎权限")
