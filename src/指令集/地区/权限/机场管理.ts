...
import { 更新地区资料, 地区机场权限设置检查 } from "@/utils";
...
function 解析机场权限模式(输入值?: string): 机场权限模式 | null {
	...
	if (
		["允许", "开放", "允许全部", "全部", "是", "开", "on"].includes(标准输入)
	) {
		return "允许全部";
	}
...
export function 设置机场权限(ctx: Context) {
	ctx
		.command("设置机场权限 <权限模式:string> [地区编号:string]")
		.alias("城市机场权限")
		.alias("地区机场权限")
		.alias("地区机场权限")
		.alias("机场权限")
		.action(async ({ session }, 权限模式参数, 地区编号参数) => {
			try {
				...
				return await 执行机场权限设置(ctx, session, 地区编号参数, 权限模式);
			} catch (error) {
	...
	ctx
		.command("允许机场使用 [地区编号:string]")
		.alias("开启机场公用")
		.alias("机场公用")
		.action(async ({ session }, 地区编号参数) => {
			try {
				return await 执行机场权限设置(ctx, session, 地区编号参数, "允许全部");
			} catch (error) {
				return (error as Error).message;
			}
		});
	...
	ctx
		.command("禁止机场使用 [地区编号:string]")
		.alias("开启机场私用")
		.alias("机场私用")
		.action(async ({ session }, 地区编号参数) => {
			try {
				return await 执行机场权限设置(ctx, session, 地区编号参数, "仅联军");
			} catch (error) {
				return (error as Error).message;
			}
		});
