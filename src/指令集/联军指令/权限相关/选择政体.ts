    ...
    ctx.command("选择政体 <政体:string>")
        ...
        .action(async ({ session }, 政体) => {
            try {
                ...
                if (当前政体 === 联军政体.民主制 && 目标政体 === 联军政体.极权制) {
                    const 前六总贡献 = 获取排除成员后前N贡献总和(联军资料, uid, 6);
                    if (我的贡献 <= 前六总贡献) {
                        return `从民主制变更为极权制失败：你的贡献（${格式化(我的贡献)}）必须大于前六成员总贡献（${格式化(前六总贡献)}）`;
                    }
                }

                if (当前政体 === 联军政体.民主制 && 目标政体 === 联军政体.威权制) {
                    const 平均贡献 = 获取排除成员后平均贡献(联军资料, uid);
                    if (我的贡献 <= 平均贡献) {
                        return `从民主制变更为威权制失败：你的贡献（${格式化(我的贡献)}）必须大于其他成员平均贡献（${格式化(平均贡献)}）`;
                    }
                }
