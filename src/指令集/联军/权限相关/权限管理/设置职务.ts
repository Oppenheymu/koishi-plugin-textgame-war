...
export function 设置职务(ctx: Context) {
    ctx.command("设置总理 <目标:string>").action(async ({
        session
    }, 目标) => {
        try {
            const {
...
`.trim();
    ...
    ctx.command("设置元首 <目标:string>").action(async ({
        session
    }, 目标) => {
        try {
            const {
