...
export function 分配军队(ctx: Context) {
  ctx
    .command("分配军队 <数量:number> [目标:string]")
    .alias("联军分配军队")
    .action(async ({ session }, 数量, 目标) => {
      try {
        ...
        const 权限等级需求 = await 玩家联军权限设置(ctx, session, "分配军队");
        const { id, uid, username, 用户资料, 联军编号, 联军资料 } = await 玩家联军检查(ctx, session, {
          最低权限等级: 权限等级需求,
          是否必须在成员列表: true,
        });
        ...
        const 输入目标 = 目标?.trim();
        if (输入目标) {
          const { 目标用户ID, 目标用户名, 目标用户资料 } = await 目标解析(ctx, session, 输入目标);
          if (目标用户资料.所在联军 !== 联军编号) {
        ...
        if ((联军资料.联军军队 ?? 0) < 数量) {
          throw new Error(`联军军队不足：当前 ${格式化(联军资料.联军军队 ?? 0)}，需求 ${格式化(数量)}`);
        }
        ...
        if (当前贡献 < 数量) {
          throw new Error(`目标贡献不足：当前 ${格式化(当前贡献)}，至少需要 ${格式化(数量)}`);
        }
