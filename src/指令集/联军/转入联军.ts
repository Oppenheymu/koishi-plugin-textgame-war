...
import { 玩家联军检查, 更新玩家资料 } from "@/utils";
  ...
  ctx
    ...
    .action(async ({ session }, 数量) => {
      try {
        ...
        if ((用户资料.私人军队 ?? 0) < 数量) {
          throw new Error(`私人军队不足：当前 ${格式化(用户资料.私人军队 ?? 0)}，需求 ${格式化(数量)}`);
        }

        const 扩军计划上限 = 联军资料.扩军计划;
        if (typeof 扩军计划上限 === "number" && 扩军计划上限 > 0 && 数量 > 扩军计划上限) {
          throw new Error(`当前联军扩军计划上限为 ${格式化(扩军计划上限)}，本次最多转入 ${格式化(扩军计划上限)}`);
        }
