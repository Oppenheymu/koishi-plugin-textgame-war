...
import type { 支持平台 } from "#shared/kernel/跨域类型";
...
export function 用户检查(session: Session | undefined): asserts session is Session {
    会话检查(session);
...
export function 发送并抛出错误(session: Session, 用户提示: string, 错误信息 = 用户提示): never {
