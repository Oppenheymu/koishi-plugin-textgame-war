...
import type { Army, Battle, Region, 军队面板 } from "#/types";
import {
    HP伤害系数,
    HP伤害骰子众数,
    HP伤害骰子最大,
...
import { 获取相邻地区 } from "#/地理集";
import { 聚合军队面板, 计算HP衰减系数, 计算经验修正 } from "./属性聚合";
import { 计算战场宽度, 计算攻击地形地貌修正, 计算超宽惩罚 } from "./战场宽度";
import {
    HP分级,
    加载联军名称缓存,
    向地区绑定群推送,
...
import { 计算行军毫秒数, 选择撤退目的地 } from "./行军计算";
...
function 执行一方输出(参数: {
    ...
    const 目标硬度 =
        目标方总兵力 > 0
            ? 目标方.reduce((和, t) => 和 + t.面板.硬度 * t.军队.士兵数量, 0) /
              目标方总兵力
            : 0;
    ...
    for (const 攻击方 of 开火方) {
        ...
        if (目标面板.组织度 > 0) {
            ...
            主目标.军队.当前组织度比例 = Math.max(
                0,
                主目标.军队.当前组织度比例 - 组织度比例损失,
            );
            主目标.本轮组织度损失 += 组织度比例损失;
        }
        if (目标面板.HP > 0) {
            const HP比例损失 = HP伤害 / 目标面板.HP;
            主目标.军队.当前HP比例 = Math.max(
                0,
                主目标.军队.当前HP比例 - HP比例损失,
            );
            主目标.本轮HP损失 += HP比例损失;
        }
        攻击方.本轮造成组织度伤害 += 组织度伤害;
...
async function 结算单场战斗(ctx: Context, 战斗: Battle): Promise<void> {
    ...
    // 预备队（未上场）同步经验与状态写回
    const 上场编号集合 = new Set(
        [...攻方统计, ...守方统计].map((t) => t.军队.id),
    );
...
async function 结束战斗(
    ...
    const 取军队 = (item: Army | 军队本轮统计): Army =>
        "军队" in item ? item.军队 : item;
