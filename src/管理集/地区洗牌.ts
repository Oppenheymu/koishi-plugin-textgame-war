...
export function 地区洗牌(ctx: Context) {
    ctx.command('地区洗牌', { authority: 4 })
        .action(async ({ session }) => {
            ...
            try {
                // 1. 获取数据
                const 所有陆地 = await ctx.database.get('马列地区地形表', { 是否为海洋: false }, ['地区编号']);
                ...
                for (let 倒序索引 = 陆地编号列表.length - 1; 倒序索引 > 0; 倒序索引--) {
                    ...
                    // 交换元素
                    [陆地编号列表[倒序索引], 陆地编号列表[随机索引]] = [陆地编号列表[随机索引], 陆地编号列表[倒序索引]];
                }

                // 3. 重置数据库状态
                await ctx.database.remove('马列地区洗牌池', {});
                await ctx.database.upsert('马列服务表', [{ id: 'service', 当前地区洗牌指针: 0 }]);

                // 4. 分块写入数据库
                const 分块大小 = 1000;
                for (let 当前进度 = 0; 当前进度 < 陆地编号列表.length; 当前进度 += 分块大小) {
                    // 切割数据块并映射格式
                    const 数据块 = 陆地编号列表.slice(当前进度, 当前进度 + 分块大小).map((编号, 块内索引) => ({
                        id: 当前进度 + 块内索引,
                        地区编号: 编号
                    }));
