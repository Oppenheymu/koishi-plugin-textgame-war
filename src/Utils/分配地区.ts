...
export async function 分配坐标逻辑(ctx: Context, id: number) {
    ...
    const 已遍历池 = await ctx.database.get('马列地区洗牌池', { id: { $lt: 当前指针 } }, ['地区编号'])
    const 已遍历编号 = 已遍历池.map(记录 => 记录.地区编号)

    // 【优化】必须判断数组长度，某些数据库直接传入空数组 $in: [] 会报错
    if (已遍历编号.length > 0) {
        const [回收地] = await ctx.database.get('马列地区状态机', {
            地区编号: { $in: 已遍历编号 },
            是否已分配: false
        }, { limit: 1 })
    ...
    // --- 第二步：正常发新地 (如果没有捡到漏) ---
    if ( !目标编号 ) {
        const [新地区] = await ctx.database.get('马列地区洗牌池', { id: 当前指针 })
        if (!新地区) return '所有地区已领完！'

        目标编号 = 新地区.地区编号

        // 只有发新地才推指针
        await ctx.database.set('马列服务表', { id: 'GLOBAL' }, {
            当前地区洗牌指针: 当前指针 + 1
        })
    }
    ...
    await ctx.database.set('马列地区状态机', { 地区编号: 目标编号 }, {
        地区归属国: id,
        是否已分配: true,
    })
